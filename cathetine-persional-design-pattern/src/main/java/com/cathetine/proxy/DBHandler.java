...
public class DBHandler<T> implements InvocationHandler {
    ...
    public T get() {
         ...
         * 第一个参数：dbBaseStrategy.getClass().getClassLoader()，使用handler对象的classloader对象来加载我们的代理对象
         * 第二个参数：dbBaseStrategy.getClass().getInterfaces()，这里为代理类提供的接口是真实对象实现的接口，这样代理对象就能像真实对象一样调用接口中的所有方法
